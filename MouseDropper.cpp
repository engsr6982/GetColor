...
static const int    G_GrabInterval       = 50;  //刷新频率
...
MouseDropper::MouseDropper(QWidget* parent) : QWidget(parent) {
    // 创建定时器用于定期更新取色器位置
...
void MouseDropper::paintEvent(QPaintEvent* ev) {
    ...
    // 截取指定区域的屏幕图像
    QPixmap grab = screen->grabWindow(0, grabRect.x(), grabRect.y(),
    ...
    // 获取鼠标位置的像素颜色
    QPixmap pixmap = grab.copy(G_WindowSize.width() / G_MagnificationTimes / 2, G_WindowSize.height() * G_Split / G_MagnificationTimes / 2,1,1);//截图1个像素
    ...
    // 绘制窗口下部的颜色信息显示区域
    painter.fillRect(0, G_WindowSize.height() * G_Split, G_WindowSize.width() - 1, G_WindowSize.height() * (1 - G_Split), QColor(100, 100, 100));
    ...
    painter.drawText(32, 95, QString().asprintf("%d,%d,%d",(mColor.red() - 1) * 255 / 254, (mColor.green() - 1) * 255 / 254, (mColor.blue() - 1) * 255 / 254));
}
